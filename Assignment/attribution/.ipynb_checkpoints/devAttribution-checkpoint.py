...
# %%
def explore_data(train_df, test_df, target_column):
    """
    ...
    sns.countplot(data=train_df, x=target_column)
    ...
    for i, col in enumerate(numerical_cols[:min(len(numerical_cols), 9)], 1):
        plt.subplot(3, 3, i)
    ...
    for col in numerical_cols[:5]:  # Showing first 5 features
        plt.figure(figsize=(10, 6))
        sns.boxplot(data=train_df, x=target_column, y=col)
    ...
    for col in numerical_cols:
        try:
            statistic, p_value = stats.f_oneway(
                *[group[col].values for name, group in train_df.groupby(target_column)]
            )
            print(f"\n{col}:")
            print(f"F-statistic: {statistic:.4f}")
            print(f"p-value: {p_value:.4f}")
        except:
            print(f"\nCould not perform F-test for {col}")
...
eda_results = explore_data(train_df, test_df, target_column)

# You can use the results later in your code
print("\nEDA Summary:")
print(f"Number of numerical features: {eda_results['num_features']}")
print(f"Features with missing values: {len(eda_results['missing_values'])}")
print(f"Number of classes: {len(eda_results['target_distribution'])}")
...
# Remove 'label' column from test data
if 'attribution' in test_df.columns:
...
print("\nModel trained on full training dataset with top features")

# Make predictions on test set with top features (Random Forest):
test_predictions_top = rf_model_top.predict(test_df_top)
print("\nRandom Forest Performance on Test Set (top features):")
print("\nClassification Report:")
print(classification_report(test_labels, test_predictions_top))
rf_accuracy_top = accuracy_score(test_labels, test_predictions_top)

# Bagging Classifier predictions
bag_test_predictions = bag_clf.predict(test_df_top)
print("\nBagging Classifier Performance on Test Set (top features):")
print("\nClassification Report:")
...
print(f"Random Forest Accuracy (full): {rf_accuracy:.4f}")
print(f"Random Forest Accuracy (top features): {rf_accuracy_top:.4f}")
...
# Add size-based features (since many top features are related to packet size)
if 'min_packet_size' in X_enhanced.columns and 'max_packet_size' in X_enhanced.columns:
    X_enhanced['packet_size_range'] = X_enhanced['max_packet_size'] - X_enhanced['min_packet_size']
    test_df_enhanced['packet_size_range'] = test_df_enhanced['max_packet_size'] - test_df_enhanced['min_packet_size']

# Add bandwidth-based features
bandwidth_features = [col for col in X.columns if 'bandwidth' in col]
if bandwidth_features:
    X_enhanced['total_bandwidth'] = X[bandwidth_features].sum(axis=1)
    test_df_enhanced['total_bandwidth'] = test_df[bandwidth_features].sum(axis=1)

# Add packet size statistics from first_packet_sizes
packet_size_features = [col for col in X.columns if 'first_packet_sizes' in col]
if packet_size_features:
    X_enhanced['avg_first_packet_size'] = X[packet_size_features].mean(axis=1)
    test_df_enhanced['avg_first_packet_size'] = test_df[packet_size_features].mean(axis=1)
...
# 3. Feature Groups
size_features = [col for col in X.columns if 'size' in col.lower()]
bandwidth_features = [col for col in X.columns if 'bandwidth' in col.lower()]
packet_features = [col for col in X.columns if 'packet' in col.lower()]
grouped_features = size_features + bandwidth_features + packet_features
